...
public class StepsView extends View {
    ...
    private void initVariable() {
        ...
        mRingPaintBg.setStrokeCap(Paint.Cap.ROUND);
...
//        canvas.drawArc(oval1, 0, 360, false, mRingPaintBg); //圆弧所在的椭圆对象、圆弧的起始角度、圆弧的角度、是否显示半径连线
        canvas.drawArc(oval1, 60, -300, false, mRingPaintBg); //圆弧所在的椭圆对象、圆弧的起始角度、圆弧的角度、是否显示半径连线
        ...
        mTxtWidth = mTextPaint.measureText(txt, 0, txt.length());
        canvas.drawText(txt, mXCenter - mTxtWidth / 2, mYCenter + mRadius, mTextPaint);
    }
    ...
    public synchronized int getmTotalProgress() {
        return mTotalProgress;
    }
    public synchronized void setmTotalProgress(int max) {
        if(max < 0){
