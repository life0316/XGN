...
public class HxRoundProgress extends View {
    ...
    public HxRoundProgress(Context context) {
        this(context,null);
    }

    public HxRoundProgress(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public HxRoundProgress(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        paint  =  new Paint();
        //从attrs.xml中获取自定义属性和默认值
        TypedArray typedArray  = context.obtainStyledAttributes(attrs, R.styleable.HxRoundProgress);
        if (typedArray != null) {
            roundColor = typedArray.getColor(R.styleable.HxRoundProgress_roundColor, Color.GREEN);
            roundProgressColor = typedArray.getColor(R.styleable.HxRoundProgress_roundProgressColor, Color.RED);
            innerRoundColor = typedArray.getColor(R.styleable.HxRoundProgress_innerRoundColor, Color.WHITE);
            roundWidth = typedArray.getDimension(R.styleable.HxRoundProgress_roundWidth, 30);
            textColor  =typedArray.getColor(R.styleable.HxRoundProgress_textColor, Color.RED);
            textSize = typedArray.getDimension(R.styleable.HxRoundProgress_textSize, 15);
            max = typedArray.getInteger(R.styleable.HxRoundProgress_max, 100);
            style = typedArray.getInt(R.styleable.HxRoundProgress_style, STROKE);
            isDisplayText  =typedArray.getBoolean(R.styleable.HxRoundProgress_textIsDisplayable, true);
            typedArray.recycle();
        }
    }
